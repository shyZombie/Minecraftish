...
public class GenerateLandscape : MonoBehaviour {
	...
	void CreateBlock (int y, Vector3 blockPos, bool create)
	{
		...
		//Random.Range(0,100) < 10 - means 10% chance to find diamond block
		if (y > 45 && y < 53 && Random.Range(0,100) < 10) 
		{
			if (create) 
			{
	...
	void Update () 
	{
		if (Input.GetMouseButtonDown (0)) 
		{
			...
			if (Physics.Raycast (ray, out hit, 11.0f))
			{
				...
				//the bottom block wouldn't be deleted!
				if ((int)blockPos.y == 0)
					return;
